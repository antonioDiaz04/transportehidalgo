...
const scoringSchema = {
    ...
    scored: [
        ...
        { key: "aireAcondicionado", label: "Aire Acondicionado", type: "select" as const, options: aireAcondicionadoOptions }, // Descomentado
...
export default function InspeccionRevistaVehicularForm() {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        const allEssentialFieldsAreGood = scoringSchema.essential.every(field => {
            ...
            if (field.type === "checkbox") {
                ...
                return value === "SI:BIEN"; // Select must be "SI:BIEN"
            }
        ...
        console.log("Todas las validaciones previas pasaron. Mostrando modal de confirmación.");
    ...
    const handleConfirmSave = async () => {
        ...
        try {
            ...
            if (imagesToUpload.length > 0) {
                ...
                for (const img of imagesToUpload) {
                    ...
                    try {
                        ...
                        setSelectedImages(prev => prev.map(sImg => sImg.id === img.id ? { ...sImg, idImagenRevistaVehicular: idRV /* Placeholder */ } : sImg));
            ...
            router.push(`/concesion/lista`); // Example: navigate back to concession list
    ...
    return (
        <div className="flex flex-col min-h-screen bg-gray-100 dark:bg-gray-900">
            <header className="sticky top-0 z-40 w-full border-b bg-white dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <CardContent className="space-y-4 text-gray-700 dark:text-gray-300">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                                <div className="flex items-center gap-2">
                                    ...
                                    <div className={`font-bold text-lg px-3 py-1 rounded-md border-2 ${getClassificationStyles(classification)}`}>
                                        {classification}
                                    </div>
                                </div>
                                <div className="flex items-center gap-2">
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                                    {selectedImages.map((image) => (
                                        <Card key={image.id} className="relative group overflow-hidden border-gray-300 dark:border-gray-600">
                                            <Image
                                            ...
                                            <CardContent className="p-3">
                                                <Label htmlFor={`type-${image.id}`} className="sr-only">Tipo de imagen</Label>
                                                <Select
                                                    value={image.type}
                                                ...
                                                {image.customName && (
                                                    <p className="text-xs text-gray-500 mt-1 truncate">{image.customName}</p>
                                                )}
                            ...
                            <CardContent className="space-y-4">
                                ...
                                {isRejected && (
                                    <p className="text-sm text-red-600 dark:text-red-400 flex items-center gap-1">
                                        <TriangleAlert className="h-4 w-4" /> La aprobación está deshabilitada ya que el vehículo fue RECHAZADO.
                                    </p>
                                )}
                            </CardContent>
                        </Card>
            ...
            <AlertDialog open={inspectionResultAlert?.isOpen || false} onOpenChange={(open) => setInspectionResultAlert(prev => prev ? { ...prev, isOpen: open } : null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                    ...
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            ...
                            <p className="text-gray-700 dark:text-gray-300">
                                Estos campos son **críticos**. Si cualquiera de ellos (checkbox o select) no cumple con la condición de "BIEN" (marcado para checkboxes, "SI:BIEN" para selects), el vehículo es automáticamente **RECHAZADO**. La puntuación no se calcula si el vehículo es rechazado.
                            </p>
                            <h3 className="font-bold text-gray-900 dark:text-white mt-4 mb-2">Campos Puntuables ("Características para el Tipo de Servicio"):</h3>
                            <p className="text-gray-700 dark:text-gray-300">
                                Estos campos suman puntos basados en la opción seleccionada. La puntuación total determina la clasificación final del vehículo si no ha sido rechazado por los campos esenciales.
                            </p>
                            ...
                            <ul className="list-disc pl-5 text-gray-700 dark:text-gray-300 space-y-1">
                                ...
                                <li>**NO CLASIFICADO (ID: {classificationToIdMap["NO CLASIFICADO"]}):** Si no hay rechazo pero la puntuación de los campos puntuables es 0, y todos los campos puntuables han sido seleccionados.</li>
                                <li>**PENDIENTE DE CLASIFICAR (ID: {classificationToIdMap["PENDIENTE DE CLASIFICAR"]}):** Si no hay rechazo pero no todos los campos puntuables han sido seleccionados (mantienen "0" o "invalid-selection").</li>
