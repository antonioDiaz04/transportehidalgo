...
export default function TitularModule() {
  ...
  // Añade este nuevo estado a tu componente
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    setIsSearching(true);
    setError(null);
    setConcesionariosEncontrados([]); // Limpiar resultados anteriores
    setConcesionarioSeleccionado(null); // Limpiar selección anterior
    ...
    try {
      ...
      // Se comprueba que la respuesta tenga una propiedad `data` y que sea un array
      const searchResults = data || [];

      if (searchResults.length > 0) {
        setConcesionariosEncontrados(searchResults);
        console.log("Concesionarios encontrados:", searchResults);

        // Si solo hay un resultado, lo seleccionamos automáticamente
        if (searchResults.length === 1) {
          console.log("Solo un concesionario encontrado, seleccionando automáticamente:", searchResults[0]);
  ...
  const handleSelectConcesionario = async (concesionario: any) => {
    ...
    try {
      ...
      // Desactiva el estado de carga una vez que todo haya terminado
